...
const MyContact = () => {
  return (
      <MyHomeSection theme={tLightTheme} title={"Get in Touch!"}>

        <div align={"center"}>
          {myContacts.map(value => {
            return (
                <IconButton href={value.url} target={"_blank"} style={{
                  margin: "clamp(5px, 2.5vw, 35px)",
                }}>
                  <value.icon style={{...listItemIconIconStyle.large, color: value.color}}/>
                </IconButton>
            );
          })}
        </div>
